...
export default function ApprovalsPage() {
  ...
  async function load() {
    ...
    if (!user) return
    const [ap, cl, pr] = await Promise.all([
  ...
  async function upload() {
    ...
    if (form.file) {
      ...
      if (up) {
        const { data: url } = supabase.storage.from('pieces').getPublicUrl(path)
        fileUrl = url.publicUrl; fileName = form.file.name
      }
    }

    const { data: na } = await supabase.from('approvals').insert({
      agency_id: user.id, client_id: form.client_id,
      piece_name: form.piece_name, piece_type: form.piece_type,
      file_url: fileUrl, file_name: fileName,
    }).select('*, client:clients(name,avatar_color,email)').single()
    ...
    if (na) {
      ...
      if (cl?.email) {
        await fetch('/api/approvals/notify', {
          ...
          body: JSON.stringify({
            clientEmail: cl.email, clientName: cl.name,
            pieceName: form.piece_name, pieceType: form.piece_type,
            approvalToken: na.client_token,
          }),
        })
      }
  ...
  async function approve(id: string) {
    ...
    await supabase.from('agreements').insert({
      agency_id: user.id, client_id: a.client_id, approval_id: id,
      piece_name: a.piece_name, action: 'approved',
      client_name: cl?.name || 'Cliente', client_email: cl?.email,
    })
    ...
    await fetch('/api/approvals/team-notify', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        approvalId: id, action: 'approved',
        clientName: cl?.name, agencyId: user.id,
      }),
    })

    setApprovals(prev => prev.map(x => x.id === id ? { ...x, status: 'approved' } : x))
    showToast('✓ Aprobado — equipo notificado automáticamente')
  }
  ...
  async function reqChanges(id: string) {
    ...
    await fetch('/api/approvals/team-notify', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        approvalId: id, action: 'changes_requested',
        clientName: cl?.name, agencyId: user.id,
      }),
    })

    setApprovals(prev => prev.map(x => x.id === id ? { ...x, status: 'changes' } : x))
    showToast('Marcado para revisión — equipo notificado')
  }
  ...
  const filtered = approvals.filter(a =>
    ...
    tab === 'changes' ? a.status === 'changes' :
    ['approved','revised'].includes(a.status)
  )
  const counts = {
    pending: approvals.filter(a => a.status === 'pending').length,
    changes: approvals.filter(a => a.status === 'changes').length,
    approved: approvals.filter(a => ['approved','revised'].includes(a.status)).length,
  }
    ...
    <div style={s.page}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
        <div>
          ...
          <p style={{ fontSize: '13px', color: '#64748B', marginTop: '4px' }}>Sin intermediarias — el equipo se entera automáticamente</p>
        </div>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid #E2E0D8', marginBottom: '20px' }}>
        {(['pending','changes','approved'] as Tab[]).map((t, i) => (
          <button key={t} onClick={() => setTab(t)}
            style={{ padding: '10px 20px', border: 'none', borderBottom: `2px solid ${tab === t ? '#E8623A' : 'transparent'}`, background: 'transparent', cursor: 'pointer', fontSize: '13px', fontWeight: 600, fontFamily: 'system-ui,sans-serif', color: tab === t ? '#E8623A' : '#64748B' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: '16px' }}>
          {filtered.map(a => {
            ...
            return (
              <div key={a.id} style={{ background: '#fff', border: '1px solid #E2E0D8', borderRadius: '12px', overflow: 'hidden' }}>
                ...
                <div style={{ padding: '12px' }}>
                  ...
                  {['approved','revised'].includes(a.status) && (
                    <div style={{ fontSize: '10px', color: '#94A3B8' }}>✓ Equipo notificado · Registrado en Acuerdos</div>
                  )}
                </div>
              </div>
            )
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '20px' }}>
              ...
              <div>
                ...
                <div onClick={() => fileRef.current?.click()} style={{ border: '2px dashed #E2E0D8', borderRadius: '8px', padding: '16px', textAlign: 'center', cursor: 'pointer' }}>
                  {form.file ? <div style={{ fontSize: '13px', fontWeight: 600, color: '#0D9488' }}>✓ {form.file.name}</div> : <div style={{ fontSize: '12px', color: '#94A3B8' }}>Clic para subir · JPG, PNG, PDF, MP4</div>}
