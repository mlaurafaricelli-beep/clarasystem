...
export default function TeamPage() {
  ...
  async function load() {
    ...
    const [m, t, c, p] = await Promise.all([
      ...
      supabase.from('tasks').select('*, client:clients(name), assignee:team_members(full_name,email)').eq('agency_id', user.id).order('due_date'),
  ...
  async function assignTask(memberId: string) {
    ...
    load()
    showToast(`✓ Tarea asignada${member?.email ? ' — Email enviado a ' + member.email : ''}`)
  }
    ...
    <div style={s.page}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
        <div>
          ...
          <p style={{ fontSize: '13px', color: '#64748B', marginTop: '4px' }}>Asignaciones y notificaciones automáticas</p>
        </div>
      ...
      {members.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '80px', color: '#94A3B8' }}>
          ...
          <button onClick={() => setShowAdd(true)} style={{ ...s.btn, background: '#E8623A', color: '#fff', padding: '10px 20px' }}>
              ...
              <div key={m.id} style={s.card}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: '11px', color: '#94A3B8' }}>
                      {m.role || 'Sin rol definido'} · {m.email}
                    </div>
                  </div>
                  ...
                  <button onClick={() => removeMember(m.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#CBD5E1', fontSize: '16px', flexShrink: 0 }}>×</button>
                </div>
                ...
                <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
                  {[
                    ...
                    <div key={stat.label} style={{ flex: 1, textAlign: 'center', padding: '6px 4px', background: '#F8F7F4', borderRadius: '6px' }}>
                      <div style={{ fontSize: '16px', fontWeight: 700, color: stat.color }}>{stat.value}</div>
                      <div style={{ fontSize: '9px', color: '#94A3B8', marginTop: '1px' }}>{stat.label}</div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ marginBottom: '10px', maxHeight: '160px', overflowY: 'auto' }}>
                  {mt.length === 0 ? (
                    ...
                    const statusColor = t.status === 'done' ? '#059669' : isLate ? '#DC2626' : '#94A3B8'
                    return (
                      <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 0', borderBottom: '1px solid #F8F7F4' }}>
                        <button onClick={() => toggleTask(t.id, t.status)}
                          style={{ background: 'none', border: 'none', cursor: 'pointer', color: statusColor, fontSize: '13px', flexShrink: 0, padding: 0 }}>
                          {statusIcon}
                        </button>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: '12px', textDecoration: t.status === 'done' ? 'line-through' : 'none', color: t.status === 'done' ? '#94A3B8' : '#0F172A', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {t.title}
                          </div>
                          {(t.client as any)?.name && (
                            <div style={{ fontSize: '10px', color: '#94A3B8' }}>{(t.client as any).name}</div>
                          )}
                        </div>
                        {t.due_date && (
                          <div style={{ fontSize: '10px', color: isLate ? '#DC2626' : '#94A3B8', flexShrink: 0 }}>
                            {new Date(t.due_date).toLocaleDateString('es-AR', { day: 'numeric', month: 'short' })}
                          </div>
                        )}
                        <button onClick={() => deleteTask(t.id)}
                          style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#E2E0D8', fontSize: '14px', flexShrink: 0, padding: 0 }}>
                          ×
                        </button>
                      </div>
                    )
                ...
                <button onClick={() => { setShowTask(m.id); setTaskForm({ title: '', client_id: '', due_date: '', priority: 'normal', description: '' }) }}
                  style={{ ...s.btn, width: '100%', background: '#F1EFE8', color: '#475569', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                  + Asignar tarea
                  {m.email && <span style={{ fontSize: '10px', color: '#94A3B8' }}>· se notifica por email</span>}
                </button>
                ...
                {showTask === m.id && (
                  <div style={{ marginTop: '10px', padding: '14px', background: '#F8F7F4', borderRadius: '10px' }}>
                    <div style={{ fontSize: '12px', fontWeight: 700, color: '#475569', marginBottom: '10px' }}>
                      📋 Nueva tarea para {m.full_name}
                    </div>
                    <input value={taskForm.title} onChange={e => setTaskForm(f => ({...f, title: e.target.value}))}
                      placeholder="Nombre de la tarea *"
                      style={{ ...s.input, marginBottom: '8px', fontSize: '12px' }} />
                    <textarea value={taskForm.description} onChange={e => setTaskForm(f => ({...f, description: e.target.value}))}
                      placeholder="Descripción (opcional)"
                      rows={2}
                      style={{ ...s.input, resize: 'none', marginBottom: '8px', fontSize: '12px' }} />
                    ...
                    <select value={taskForm.priority} onChange={e => setTaskForm(f => ({...f, priority: e.target.value}))}
                      style={{ ...s.input, fontSize: '12px', marginBottom: '10px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button onClick={() => setShowTask(null)}
                        style={{ flex: 1, padding: '8px', borderRadius: '8px', border: '1px solid #E2E0D8', background: '#fff', cursor: 'pointer', fontSize: '12px', fontFamily: 'system-ui,sans-serif' }}>
                        Cancelar
                      </button>
                      <button onClick={() => assignTask(m.id)} disabled={!taskForm.title}
                        style={{ flex: 1, padding: '8px', borderRadius: '8px', border: 'none', background: '#E8623A', color: '#fff', cursor: 'pointer', fontSize: '12px', fontWeight: 600, fontFamily: 'system-ui,sans-serif', opacity: !taskForm.title ? 0.5 : 1 }}>
                        Asignar y notificar
                      </button>
                    </div>
                  </div>
          ...
          <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', width: '100%', maxWidth: '400px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setShowAdd(false)}
                style={{ flex: 1, padding: '10px', borderRadius: '8px', border: '1px solid #E2E0D8', background: '#fff', cursor: 'pointer', fontSize: '13px', fontFamily: 'system-ui,sans-serif' }}>
                Cancelar
              </button>
              <button onClick={addMember} disabled={!form.full_name || !form.email}
                style={{ flex: 1, padding: '10px', borderRadius: '8px', border: 'none', background: '#E8623A', color: '#fff', cursor: 'pointer', fontSize: '13px', fontWeight: 600, fontFamily: 'system-ui,sans-serif', opacity: !form.full_name || !form.email ? 0.5 : 1 }}>
                Agregar
              </button>
            </div>
          </div>
      ...
      {toast && (
        <div style={{ position: 'fixed', bottom: '20px', right: '20px', background: '#1E293B', color: '#fff', padding: '12px 16px', borderRadius: '10px', fontSize: '13px', fontWeight: 500, borderLeft: '3px solid #14B8A6', zIndex: 99, maxWidth: '320px' }}>
          {toast}
        </div>
      )}
    </div>
